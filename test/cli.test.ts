...
describe("cli papercuts", () => {
  ...
  it("deploy regenerates the .dockerignore it wrote itself — 'never force' protects YOUR file, not ours", async () => {
    // The root .dockerignore is the one artifact deploy writes outside the agent dir, so it is never
    // force-clobbered. Without an ownership marker that would strand OUR OWN file the day its content
    // changes: every later deploy keeps a stale ignore and preflight gates on it forever.
    ...
    expect(forced.stderr).toMatch(/kept \.dockerignore — the workspace's own file/);
