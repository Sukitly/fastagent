...
import { AGENT_DIR, resolveAuthPath, resolveSecretsDir, resolveStateRoot } from "../engines/pi/config.ts";
...
export async function preflightDeploy(input: {
  ...
  if (keptDockerignore !== undefined && !(force && isGeneratedDockerignore(keptDockerignore))) {
    ...
    if (stateRel && !excluded(`${stateRel}/sessions`)) {
      messages.push({
        level: "warn",
        text: `your .dockerignore (kept) does not exclude \`${stateRel}\` — the build machine's sessions/channel state would ship in the image. Add \`/${stateRel}\`.`,
      });
    }
