...
import { AGENT_DIR, resolveAuthPath, resolveSecretsDir } from "../engines/pi/config.ts";
...
export async function preflightDeploy(input: {
  ...
  const deps = hasPackageJson
    ? `only the agent's deps (${AGENT_DIR}/package.json) are installed — the workspace's own deps are the agent's runtime concern`
  ...
  const secretPaths = [inContext(authPath), secretsRel && `${secretsRel}/.env`].filter((p) => p !== undefined);
  const keptDockerignore = await readFile(join(workspace, ".dockerignore"), "utf8").catch(() => undefined);
  // A `.dockerignore` WE generated is regenerated by this very run under --force, so checking the
  // stale content on disk would gate a deploy on a file about to be replaced. Ours + --force: skip.
  if (keptDockerignore !== undefined && !(force && isGeneratedDockerignore(keptDockerignore))) {
    ...
    if (excluded(AGENT_DIR) && excluded(`${AGENT_DIR}/persona.md`)) {
      const text =
    ...
    // The real credential paths, not spellings: a root-anchored `.secrets` line (dockerignore patterns
    // are root-anchored, unlike .gitignore) does NOT cover the agent's own `fastagent/.secrets`. And
    // RESOLVED, not default: FASTAGENT_SECRETS_DIR / FASTAGENT_AUTH_PATH can put the credential
    // anywhere in the baked tree, where the `**/.secrets` name-based excludes would never reach it —
    // this is the one check whose failure mode is "credentials in a published image".
    const leaks = [...secretPaths, ".env"].filter((p) => !excluded(p));
    if (leaks.length > 0) {
      const text =
    ...
    if (!excluded(`${AGENT_DIR}/.state/sessions`)) {
      messages.push({
        level: "warn",
        text: `your .dockerignore (kept) does not exclude \`${AGENT_DIR}/.state\` — the build machine's sessions/channel state would ship in the image. Add \`**/.state\`.`,
      });
    }
