 ...
 * `lambda/forwarder.mjs` artifact are generated from (one source, no drift). Zero-dependency: the
 * Lambda Node runtime bundles AWS SDK v3. Two event shapes: a Function URL webhook (reconstructed
 * verbatim into a `webhook` envelope; the channel's REAL response rides back inside the transport
 * reply and is re-emitted byte-exact — Feishu's URL-verification challenge depends on it), and an
 * EventBridge Scheduler fire (`{ scheduleFire }`, slot = the scheduled instant — the container's
 * idempotency key). MUST stay under CloudFormation's 4096-byte inline-code cap.
 */
...
function template(input: AgentcorePlanInput, translated: { fact: ScheduleFact; expression: string }[]): string {
  ...
  if (needsForwarder) {
    lines.push(
      ...
      `      Code:`,
      `        ZipFile: | # same source as lambda/forwarder.mjs (generated together — cannot drift)`,
...
export function planAgentcoreDeploy(input: AgentcorePlanInput): AgentcorePlan {
  ...
  const artifacts: Artifact[] = [
    ...
    ...(needsForwarder ? [{ path: `${prefix}lambda/forwarder.mjs`, content: forwarderSource() }] : []),
