...
// after firing. Upsert (create → conflict → update); a past instant is skipped (the box is awake
// handling it — that's how the fireAt got claimed/advanced). Cancelled wakes are NOT deleted here:
// their poke fires, finds nothing due, and the schedule self-deletes (lazy cleanup by design).
async function syncAlarms(alarms, ctx) {
  ...
  for (const a of alarms) {
    ...
    try {
      ...
      if (e.name === "ConflictException") await sch.send(new UpdateScheduleCommand(p)).catch((u) => console.log(\`alarm \${p.Name}: \${u}\`));
      else console.log(\`alarm \${p.Name}: \${e}\`); // past-instant/validation — logged, never fatal
    }
  }
}
...
exports.handler = async (event, ctx) => {
  ...
  if (event.rawPath === "/__fastagent/wake-alarm") {
    ...
    await syncAlarms(req.alarms || [], ctx);
