...
selects both runtime policy and manifest scopes/events. By default it opens Slack's App Configuration
Token page, creates an internal app with `apps.manifest.create`, installs it through OAuth, and writes the
Bot Token + Signing Secret to the gitignored `.env`. The configuration refresh token stays owner-readable
