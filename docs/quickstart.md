...
**The repository IS the agent?** (A standalone agent repo, or a monorepo package.) `fastagent init . --flat` puts the same shape at the root instead. Existing files are kept untouched, and the agent's workspace is its own directory, so its tools operate on its own definition. **Want a different directory name?** `fastagent init . --agentDir bot` — the name is never a rule; the `fastagent.config.*` inside is what makes a directory an agent.
