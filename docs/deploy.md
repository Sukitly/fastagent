...
For a flat agent this generates `Dockerfile`, `.dockerignore`, and `fastagent.compose.yml`. The Compose file contains one `agent` service:
...
## Serving an existing directory (nested `fastagent/` root)

When the agent dir is the default nested `./fastagent/` (the parent directory is the workspace — see [Configuration](configuration.md)), `deploy` uses the SAME semantic as flat — **bake the workspace as the image, WYSIWYG** (what you see is what ships: git or not, clean or dirty) — with the artifacts namespaced:

- **Artifacts land in the agent dir** — `fastagent/Dockerfile`, `fastagent/Dockerfile.dockerignore`, and `fastagent/fastagent.compose.yml` / `fastagent/fly.toml` / `fastagent/railway.json` — so they never collide with the host repo's own Docker/deploy files. **One host-tree write**: a `.dockerignore` at the workspace root (context-packers only read that form; it carries the machinery excludes `**/.secrets`, `**/.state`, `**/.cache`, plus `**/node_modules` and `**/.env*`, and does *not* exclude `.git`). If the host already has one it is **kept — even under `--force`** (it's the host's file), and preflight checks it specifically: missing secret excludes (the packer would bake secrets into the image) or a rule matching `fastagent` (the context would ship without the agent) **gate `--run`** and warn generate-only; a missing `**/.state`/`**/node_modules` warns, and a `.git` exclude gets a note (kills the agent's pull/push loop). Docker Compose builds from the workspace root through the namespaced file; the Fly runbook passes explicit flags (`fly deploy . --config fastagent/fly.toml --dockerfile fastagent/Dockerfile`); on Railway the build entry rides the `RAILWAY_DOCKERFILE_PATH` service variable (set with the machinery variables — fully scriptable), and pointing the service at `fastagent/railway.json` (Settings → Config-as-code — dashboard-only) is an *optional* enhancement: it adds the `/health` deploy gate, while Railway's default restart policy already matches the file's `ON_FAILURE`.
- **The image bakes the whole directory as the agent's workspace.** Only the **agent's** dependencies (`fastagent/package.json`) are installed — the host repo's own deps are the agent's runtime concern (it can install them in its workspace when a task needs them).
...
`.git` ships in the image by default (the agent's pull/push loop needs it); for a smaller image with no git needs, add a `.git` line to the generated `.dockerignore`. The git **binary** is baked in exactly when the directory ships a `.git` (either placement); a non-git directory that still needs git declares `deploy: { apt: ["git"] }` in `fastagent.config.*`.
