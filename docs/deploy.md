...
| **State goes on persistent storage** | Sessions, `auth.json`, and channel state live under one root; a redeploy that replaces the directory wipes them otherwise. | Docker/Fly/Railway mount a volume at `/data`. AgentCore has no durable mount without a VPC, so it keeps an S3 snapshot instead (below). Each sets `FASTAGENT_STATE_DIR` to match. |
